...
void AreaInfluenceUpdate( AREA_DATA* area ){
  ...
  */
  ...
  if (IS_AREA(area,AREA_LAWFUL)) {
    CABAL_DATA* pCabal = cabal_list;
    for (pCabal = cabal_list;pCabal;pCabal=pCabal->next) {
      if (IS_CABAL(pCabal, CABAL_JUSTICE)) {
	CABAL_DATA* pc = get_parent (pCabal);
	area_to_cabal(area, pc, TRUE);
	return;
      }    
    }
  }
