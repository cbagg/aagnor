...
void char_to_char( CHAR_DATA *victim, CHAR_DATA *ch , bool fAll)
{
  ...
  if ( !is_affected(vch,skill_lookup("dark shroud")) && !is_affected(vch,skill_lookup("cloak of shades")) ){
...
void do_consider( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_GOOD(ch))
      {
	if (IS_GOOD(victim)) msg = "$N greets you warmly.";
	else if (IS_NEUTRAL(victim)) msg = "$N looks very disinterested.";
	else	msg = "$N grins at you evilly.";
      }
    else if (IS_NEUTRAL(ch))
	{
	  if (IS_GOOD(victim)) msg = "$N beams a smile at you.";
	  else if (IS_NEUTRAL(victim)) msg = "$N looks just as disinterested as you.";
	  else	msg = "$N grins at you evilly.";
	}
    else
      {
	if (IS_GOOD(victim)) msg = "$N smiles, hoping you will turn from your evil path.";
	else if (IS_NEUTRAL(victim)) msg = "$N looks very disinterested.";
	else 	msg = "$N grins evilly with you.";
      }
    act( msg, ch, NULL, victim, TO_CHAR);
