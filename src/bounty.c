...
void do_bounty( CHAR_DATA* ch, char* argument ){
  ...
  if (IS_NULLSTR( arg )){
    ...
    else if (obj->pIndexData->vnum == 102) {
     send_to_char("Nobody is interested in your dolly collection.\n\r",ch);
     return;
    }
...
/* try to get the bid by name */
  ...
  /* if the bountier exists and is of rank > 40 he gets the rest of rares up to 3 */
  ...
  if (bidder && bidder->level >= 40){
    int bidder_count = get_random_rares(bidder, victim, 3, FALSE );
