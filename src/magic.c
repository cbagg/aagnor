...
int spell_expgain(int sn, int skill, int level){
  int min = skill_table[sn].min_mana / 4;
  int max = skill_table[sn].min_mana / 2;
  int val  = 0;
...
bool check_dispel( int dis_level, CHAR_DATA *victim, int sn){
  ...
  for ( af = victim->affected; af != NULL; af = af->next ){
    if ( af->type == sn ){
      if (dis_level == -99 ||
	  (!saves_dispel(dis_level,af->level,af->duration)
	   && check_phylactery(victim) < 60)){
	if (sn == gsn_duplicate){
...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_IMMORTAL(ch) && (ch->class == class_lookup("necromancer")
	 ...
	 && !IS_EVIL(ch) && deity_table[ch->pcdata->way].shrine != 2530) // allow nature DKs.
    {
...
//bonus is 1/2 value over 150, and regular over 225.
	    ...
	    if (armor >= 300) {
	      if (vch != NULL && (ch->in_room != vch->in_room || is_ghost(vch,600)))	      {
		continue;
	      }
...
void spell_calm( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
  ...
  for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room){
    ...
    else if (IS_AFFECTED2(vch,AFF_RAGE) && number_percent() < 33){
      act("$N roars in fury, and shakes off your efforts", ch, NULL, vch, TO_CHAR);
      act("Your fury sustains you through $n's attempt to calm you", ch, NULL, vch, TO_VICT);
      fSave = TRUE;
    }
...
void spell_chain_lightning(int sn,int level,CHAR_DATA *ch, void *vo,int target)
{
    ...
    if ( (paf = affect_find(victim->affected, gsn_mimic)) == NULL){
     retu += 1;
    }

    if (retu == 2){
      act("The spell fails to encounter resistance.", ch, NULL, victim, TO_CHAR);
      return;
    }

    if (paf == NULL)
      paf = affect_find(victim->affected, sn_ms);
...
void spell_curse( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    {
      if (ch->class == gcn_lich && check_phylactery(ch) == 60) {
	send_to_char("The overpowering evil of your phylactery empowers your curse.\n\r", ch);
      }
      else {
...
/* make sure we are fighting the victim when this ends */
 ...
 if (ch!=victim) { set_fighting( ch, victim );}
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  if (obj->pIndexData->vnum == OBJ_VNUM_PHYLACTERY){ 
    extract = TRUE;
    obj = create_object( get_obj_index(OBJ_VNUM_PHYLACTERY), 15);
  }
...
void spell_magic_missile( int sn, int level, CHAR_DATA *ch,void *vo,int target)
{
    ...
    for(i = UMAX(1,level/12); i > 0 && victim != NULL; i--)
    {
	    ...
	    dam /= 2;
	    send_to_char("You feel the mystic bolts strike at your armor.\n\r", victim);
	    if ( (paf = affect_find(victim->affected, sn)) == NULL) {
	      ...
	      af.type = sn;
	      af.level = level;
	      af.modifier = paf->modifier > 19? 0 : 1;
...
void spell_dispel_undead( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    dam = dice( 10 + level, 5 );
    if ( saves_spell( level, victim,DAM_HOLY,skill_table[sn].spell_type) 
	 || (victim->class == gcn_lich && check_phylactery(victim) > 50))
      {
	send_to_char("You failed.\n\r", ch);
	return;
      }
