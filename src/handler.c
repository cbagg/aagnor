...
int get_curr_stat( CHAR_DATA *ch, int stat )
{
  ...
  max = get_max_stat( ch, stat );
  cur = get_abs_stat( ch, stat );
...
bool can_see_obj( CHAR_DATA *ch, OBJ_DATA *obj )
{
  //    OBJ_DATA *light;
    ...
    if ( ch->class == gcn_lich && obj->vnum == OBJ_VNUM_PHYLACTERY)
      return TRUE;
...
/*
    ...
    if ( vector & AFF_MISSILE_SHIELD) strcat( buf, " missile_shield");
    return ( buf[0] != '\0' ) ? buf+1 : "none";
*/
}
...
void questor_update( CHAR_DATA* ch ){
  ...
  for (vch = player_list; vch; vch = vch->next_player ){
    if (vch->desc)
      act_new("Let it be known that $t has been granted the title of `#Master Questor`` of Aabahran!",
...
int get_psalm(CHAR_DATA* ch, const char* name){
  ...
  if (pos > 0){
    for (psalm = 0; psalm < MAX_PSALM; psalm ++){
	  ...
	  && pos <= max_psalm(ch))
	return psalm;
      /*checks for baptism to allow past max psalm limit*/
      else if (psalm == psalm_lookup("baptism"))
	return psalm;
      /*end baptism check*/
    }
    return 0;
