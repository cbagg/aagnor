...
bool check_defenses( CHAR_DATA *ch, 
  ...
  const int MAX_DEF = 10;	//Current maximum number of defenses
  int seed[MAX_DEF];
  int order[MAX_DEF];
  int i;
  int cur_max = MAX_DEF;
...
/* BLINK */
	...
	return TRUE;
      break;
    case 9:
      if ( check_wall( ch, victim, obj, dt, h_roll, lvl, fVirt, sn) )
	return TRUE;
      break;
    }    
/* END DEFENSES */
...
//in places that vamp wolf can sneak.
     int base_chance = 10 + (number_percent())/5;
     int haste_mod = 15;
     int dex_mod = 2;
     int lvl_mod = 1;
...
//First few checks for haste/slow:
     if (IS_AFFECTED(victim, AFF_HASTE))
       haste = TRUE;
//day is equivalent to being slow.
     if (IS_AFFECTED(victim, AFF_SLOW) || vamp_day_check(victim))
       slow = TRUE;
     if(haste && slow)
       haste = slow = FALSE;
...
//now we make a quick roll
     if (number_percent() < base_chance)
       {
	 // Check for Doomsinger
	 if (doomsingerCheck(ch)) {
	   act("A strange melody transfixes you, and the underbrush seems distant.", ch, NULL, victim, TO_VICT);
	   return FALSE;
	 }
  //we evaded!
...
{
  int chance = ch->level, iWear, cycle, phyl = check_phylactery(victim);
...
/* check prim/sec */
  ...
  else if ( change != victim->hit && phyl > 30 && get_skill(victim,skill_lookup("chill touch")) > 90) {
    damage(victim,ch,number_range(phyl/2,3*phyl/2),gsn_phylactery,DAM_COLD,TRUE);
    if (ch->fighting == NULL || ch->fighting != victim )
      return;
  }
  ...
  if ( is_affected(victim, skill_lookup("divine retribution")) && !IS_GOOD(ch)){
    ...
    if (IS_NEUTRAL(ch))
      dam2 = 2*dam2/3;
    if (ch->class == gcn_lich && check_phylactery(ch) > 50) {
      send_to_char("Your divine power seems quenched under a dark force.\n\r", victim);
    }
    else {
...
//BLINDNESS
  if ( !can_see( ch, victim ) && !fVirt)
	  ...
	  && UMAX(1,number_percent() - (get_curr_stat(ch,STAT_LUCK) - get_curr_stat(victim,STAT_LUCK))) < get_skill(ch,gsn_blind_fighting))
	{
	  ...
	  if (!is_affected(ch, gsn_battletrance) && !is_affected(ch, gsn_insight))
	    victim_ac -= 2;
	}
      else
	{
...
int dam_spell(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dam_type)
{
	...
	if ( dam > 1 && (is_affected(victim, gsn_bone_wall) && check_phylactery(victim) == 60))
	     dam -= dam / 8;
...
//END WOLF DEFENSE.
	...
	else if (is_affected(victim, gsn_shadowmeld)) {
	  if (IS_AREA(victim->in_room->area, AREA_LAWFUL)) {
	    act("$n swings wildly as you evade the blow with easy grace.", ch, NULL, victim, TO_VICT );
	    act("You swing right past $N as he moves away from your blow with easy grace.", ch, NULL, victim, TO_CHAR);
	    return FALSE;
	  }
	}
	else{
...
//PARRY CHECK
      if (PARRY_CHECK(ch, obj2, obj, dt) == -1)
...
//PARRY CHECK
      if (obj && obj2 && PARRY_CHECK(ch, obj2, obj, dt) == -1)
	bug("Error at: PARRY_CHECK\n\r", 0);
...
/* check weapon position */
    ...
    if (obj && IS_GOOD(victim) && IS_OBJ_STAT(obj,ITEM_BLESS))
      chance += 5;
    else if (obj && IS_EVIL(victim) && IS_OBJ_STAT(obj,ITEM_EVIL))
      chance += 5;
    ...
    if (is_affected(victim, gsn_harden_def)) {
      int mod = 0;
      mod = get_weapon_obj_rating(obj2, WEAPON_ATT);
      if (mod < 0)
	chance += mod * -5;
    }
...
bool check_wall( CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA* vir_obj, int dt, int h_roll, int level, bool fVirt, int sn)
{
  OBJ_DATA *obj = (vir_obj != NULL ? vir_obj : NULL);
  int chance, ini_chance, temp = 0, vict_weapon;
  char buf1[MSL],buf2[MSL],buf3[MSL];

  // Sure Hit
  if (!IS_AWAKE(victim) || !is_affected(victim, gsn_bone_wall))
    return FALSE;
  if ( (chance = get_skill(victim,gsn_bone_wall) / 4) == 0)
    return FALSE;
  ini_chance = chance;
  ...
  if (!IS_NPC(ch)){
    if (check_phylactery(victim) < 60 || vict_weapon < 100)
      chance = get_dchance(ini_chance, chance, (h_roll - victim->level/5)/4);
  ...
  if (fVirt) {
    ...
    sprintf(buf1, "Your wall of bone deflects a %s's %s.",
	    (vir_obj == NULL ? get_vir_attack(sn) : "$p"),
	    get_vir_attack(dt));
    sprintf(buf2, "$N throws aside a %s's %s with a wall of bone.",
	    (vir_obj == NULL ? get_vir_attack(sn) : "$p"),
	    get_vir_attack(dt));
    sprintf(buf3, "$N throws aside a %s's %s with a wall of bone.",
	    (vir_obj == NULL ? get_vir_attack(sn) : "$p"),
	    get_vir_attack(dt));
  }
  else {
    sprintf(buf1, "Your wall of bone deflects $n's attack.");
    sprintf(buf2, "$N throws your attack aside with a wall of bone.");
    sprintf(buf3, "$N throws aside $n's attack with a wall of bone.");
  }
  if (!doomsingerCheck(ch)){
    act (buf1, ch, vir_obj, victim, TO_VICT);
    act (buf2, ch, vir_obj, victim, TO_CHAR);
  ...
  check_improve(victim, gsn_bone_wall, TRUE, 0);
  return TRUE;
}
...
//WOLF CHECK
    ...
    if (!fWereBeast && IS_AFFECTED2(victim,AFF_RAGE)) { 
      if (get_skill(victim, gsn_bloodhaze) < 1)
	return FALSE;
      else
	chance = 3 * chance / 4;
    }
	...
	*/
    ...
    else if (!fWereBeast && IS_AFFECTED2(victim,AFF_RAGE)) {
      sprintf(buf1, "You roar in fury and ignore $n's feeble blow.");
      sprintf(buf2, "$N roars in fury and ignores your feeble blow.");
      sprintf(buf3, "$N roars in fury and ignores $n's feeble blow.");
    }
    else
      {
...
//PREDEATH
    ...
    if ( (paf = affect_find(victim->affected, gsn_soul_tear) ) != NULL) {
      if ( is_name( paf->string, ch->name) ) {
...
/* watchrooms */
  if ( (tmp_list = room->watch_vnums) == NULL)
    return;
  while (tmp_list != NULL) {
    if ( (room = get_room_index ((int)tmp_list->cur_entry)) != NULL){
      for (vch = room->people; vch != NULL; vch = vch_next){
	vch_next = vch->next_in_room;
