...
void deathblow( CHAR_DATA* ch, CHAR_DATA* victim, int type, bool fMiss ){
  ...
  case DB_STRIP:
    ...
    for ( i = 0; i < 4; i++){
      int damtype = wield ? wield->value[3] : 62;
      dam = get_wepdam( wield, ch, victim, damtype, &dam_type );
      if (fMiss)
	dam = 15 * dam / 100;
      else
	dam = 40 * dam / 100;
      ...
      dam = damage(ch, victim, dam, damtype + TYPE_HIT, dam_type, TRUE);
      if (victim->fighting == NULL)
	break;
      if (dam > 1){
	AFFECT_DATA* paf;
...
void form_mace( OBJ_DATA* obj, CHAR_DATA* ch, CHAR_DATA* vch, bool fAutoFail){
  ...
  for (wch = vch->in_room->people; wch; wch = wch_next ){
    ...
    if (!fAutoFail){
	  ...
	  && !saves_spell( ch->level + 2, wch, DAM_BASH, SPELL_AFFLICTIVE )){
...
bool forms(CHAR_DATA *ch, char *argument, AFFECT_DATA* paf){
  ...
  chance = 4 * skill / 5;
  chance += (get_curr_stat(ch, STAT_DEX) - 18) * 3;
  ...
  if (number_percent() > chance) {
    check_improve(ch, gsn_forms, FALSE, 0);
    fAutoFail = TRUE;
  }
  else {
    check_improve(ch, gsn_forms, TRUE, 0);
  }
...
void spell_guard_call( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    sprintf(buf, "Blood Guard! Fulfill your oath to Order!");
    do_yell(ch, buf);
...
void do_ensnare(CHAR_DATA *ch, char *argument)
{
    ...
    else if (victim->pCabal && get_parent(victim->pCabal)->vnum == 21 && ch->in_room->sector_type == SECT_FOREST) {
      send_to_char("Your familiarity with the forests allows you to evade the guards.\n\r", victim);
      send_to_char("Your victim vanished into the undergrowth for a moment, and the guards lose him.\n\r", ch);
      return;
    }
    else if (ch->mana < cost){
      send_to_char("You are too tired to order your guards about effectivly.\n\r", ch );
...
void spell_tesseract( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    if (IS_NPC(victim) || !IS_SET(victim->act, PLR_WANTED)) {
      send_to_char("You cannot summon Law's Gate to someone who has not sinned against the plane of Order.",ch);
      return;
    }
    if (is_affected(ch, sn)) {
      send_to_char("You cannot so soon risk passage through the planes of Order.", ch);
      return;
    }
    ...
    if (saves_spell( level + 1, victim, DAM_OTHER,skill_table[sn].spell_type )){
      send_to_char("You failed.\n\r", ch);
        ...
        if (is_same_group(wch, ch) && wch != ch)
	{
	  ...
	  act("$n utters some strange words and, with a sickening lurch, you feel the world shift.  The essense of Order flashes before you.",ch,NULL,wch,TO_VICT);
	  char_from_room(wch);
    ...
    act("As you utter the words, time and space seem to blur.  You see the purity of Order as you turn.",ch,NULL,NULL,TO_CHAR);
    char_from_room(ch);
...
{ 
	...
	for (paf = ch->affected; paf; paf = paf = paf_next){
	  paf_next = paf->next;
	...
	return;
      ...
      for (paf = ch->affected; paf; paf = paf = paf->next){
...
void spell_maze( int sn, int level, CHAR_DATA *ch, void *vo, int target ){
  ...
  else if ( saves_spell(level,victim, DAM_CHARM, SPELL_MENTAL)){
...
/* we strip diplomacy ONLY if we succeed in abduction */
  ...
  if ( victim->in_room == NULL 
       ...
       || is_affected(victim, gsn_ward_of_binding))
    {
...
void do_exile( CHAR_DATA *ch, char *argument ){
  ...
  const int skill = get_skill(ch, gsn_exile);
  ...
  if (IS_IMMORTAL(victim)){
    send_to_char("It is not wise to provoke the ire of such a creature.\n\r", ch);
    return;
  }
...
/* start the gen */
  ...
  sprintf(buf, "A royal page hands you a scroll:\n\r"\
	  ...
	  "And is no longer safeguarded by the laws of Aabahran!\n\r",
	  PERS2( ch ),
	  PERS2( victim),
	  city->name);
  ...
  if (victim->pcdata->flagged < 15) {
    victim->pcdata->flagged += 15;
    send_to_char("You are now an EXILE.\n\r", victim);
  }
...
void spell_whirlwind( int sn, int level, CHAR_DATA *ch, void *vo,int target ){
  ...
  else if (is_affected(victim, gsn_ward_of_binding)) {
    send_to_char("They are held too strongly to this city.\n\r", ch);
    act("A few strong gusts of wind whip through the area.\n\r", ch, NULL, NULL, TO_ALL);
    damage( ch, victim, 0, sn, DAM_AIR, TRUE );
    return;
  }
...
/* now move user and his charmies */
  for (vch = ch->in_room->people; vch; vch = vch_next){
	...
	&& vch->master == victim){
      if (IS_NPC(vch)
	  && vch->pIndexData->vnum == MOB_VNUM_SPECIAL_GUARD) {
	act("With a horrible scream, the Blood Guard's body is torn asunder by the rushing winds!", vch,NULL,NULL, TO_ROOM);
	raw_kill(ch, vch);
      }
      else
	spell_isolate(skill_lookup("isolate"), ch->level, ch, vch, TARGET_CHAR);
    }
  }
...
void spell_ward_of_binding( int sn, int level, CHAR_DATA *ch, void *vo,int target ){
  AFFECT_DATA af;
  ...
  send_to_char("You anchor yourself to the cities.\n\r", ch);
  act("$n's visage becomes grim and set.", ch, NULL, NULL, TO_ROOM );
...
void spell_shadowmeld( int sn, int level, CHAR_DATA *ch, void *vo,int target ){
  AFFECT_DATA af;
  ...
  send_to_char("You begin to feel comfortable in the cities.\n\r", ch);
  act("$n begins to move with an easy grace.", ch, NULL, NULL, TO_ROOM );
