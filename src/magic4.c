...
void spell_harden_def( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  AFFECT_DATA af;
  if (is_affected(ch,sn)) {
    send_to_char("Your will is already hardened.\n\r",ch);
    return;
  }
  if (IS_AFFECTED(ch,AFF_CALM)) {
    send_to_char("Your will has been calmed, and cannot be strengthened.\n\r",ch);
    return;
  }
  if (get_eq_char(ch, WEAR_SHIELD) == NULL) {
    send_to_char("You need a shield to focus your inner steel upon.\n\r",ch);
    return;
  }

  act("You feel your will become as hard as steel, and your shield crackles darkly.", ch,NULL,NULL,TO_CHAR);
  act("$n closes $s eyes and with a shimmer, $s shield crackles darkly.", ch,NULL,NULL,TO_ROOM);
...
bool eff_turn_undead(int level, CHAR_DATA *vch)
{
  ...
  if (IS_NPC(vch) 
    ...
    return FALSE;
  else if (vch->class == gcn_lich && check_phylactery(vch) > 50)
    return FALSE;
  else
    return TRUE;
}//end effect.
...
void divine_discord(CHAR_DATA* ch, int level, int sn)
{
  ...
  if (number_percent() < 45) {
    ...
    if (number_percent() < 70)
      ...
      send_to_char("You are now resistant to weapons.\n\r", ch);
  }
  else if (number_percent() < 90) {
    if (ch->race != race_lookup("fire")) {
      af.where = TO_RESIST;
  ...
  check_dispel(-99, ch, gsn_mimic);
  damage(ch,ch,number_range(level/2,level),gsn_feedback,DAM_INTERNAL,TRUE);
}
...
void spell_insight( int sn, int level, CHAR_DATA *ch, void *vo, int target ){
  ...
  af.duration   = 300;
  af.where      = TO_AFFECTS;
  af.bitvector	= AFF_DETECT_INVIS | AFF_DETECT_EVIL | AFF_DETECT_GOOD | AFF_DETECT_HIDDEN;
  af.modifier	= 0;
  af.location	= APPLY_NONE;
  affect_to_char(ch,&af);
...
void spell_negative_blast( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam = 2 * level + dice (UMIN(40, check_phylactery(ch)), 4);
  if (is_affected(ch, skill_lookup("dark heart")))
    dam += (UMIN(10, ch->pcdata->kpc) * 3) + (UMIN(3, ch->pcdata->flagged) * 10);
  if ( saves_spell( level, victim, DAM_NEGATIVE, skill_table[sn].spell_type)) {
    if (is_affected(ch, skill_lookup("dark heart"))) {
      dam /= 2;
    }
  ...
  else if ( check_phylactery(ch) >= 55 ) {
      send_to_char("You feel a rush as negative energy is transmitted through your phylactery.\n\r", ch);
      ch->hit += dam / 20;
  }
