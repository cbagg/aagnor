...
void lagprot_yell(CHAR_DATA* ch, CHAR_DATA* victim, int gsn, int shield)
{
  ...
  switch (shield)
	...
	PERS(ch,victim));
      ...
      /* WALL OF BONE */
    case LAG_SHIELD_BONE:
      sprintf(buf, "Do you really think your %s can penetrate my power, %s?", IS_GEN(gsn) ? effect_table[GN_POS(gsn)].name : skill_table[gsn].name, PERS(ch,victim));
      break;
    ...
    case LAG_SHIELD_ARMOR:
      ...
      /* MALFORM */
    case LAG_SHIELD_MALFORM:
      sprintf(buf, "Your worthless %s is no match for my dark power, %s!",
	IS_GEN(gsn) ? effect_table[GN_POS(gsn)].name : skill_table[gsn].name,
        PERS(ch,victim));
      break;
...
void check_magic_strike (CHAR_DATA* ch, CHAR_DATA* victim) {
  // check if victim has strike
  int skill;
  int number = number_range(0,5);
  int hasCast = FALSE;
  int sn = 0;
  if ((skill = get_skill(victim,gsn_magic_strike)) < 2)
    return;
  ...
  if (victim->race == race_lookup("human"))
    skill += 5;
  if ((ch->alignment > GOOD_THRESH && victim->alignment < EVIL_THRESH) ||
      (ch->alignment < EVIL_THRESH && victim->alignment > GOOD_THRESH))
    skill += 5;

  if (victim->class == gcn_dk) {
    act("Your fingers flare with mystical power as you strike with magic potency.", victim,NULL,ch,TO_CHAR);
    act("$n's fingers flare with mystical power as he strikes with magic potency.", victim,NULL,ch,TO_VICT);
    act("$n's fingers flare with mystical power as he strikes $N with magic potency.", victim,NULL,ch,TO_NOTVICT);
    switch (number)
      {
      case (0) :  // Curse
	if (!is_affected(ch, gsn_curse)) {
	  if (get_skill(victim, gsn_curse) > 99)
	    spell_curse(gsn_curse, victim->level,victim, (void*)ch,TARGET_CHAR);
	  else {
	    act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	  }
	  hasCast = TRUE;
	}
	break;
	...
	if (!is_affected(ch, gsn_poison)) {
	  if (get_skill(victim, gsn_poison) > 99)
	    spell_poison(gsn_poison, victim->level, victim, (void*)ch,TARGET_CHAR);
	  else {
	    act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	  }
	  hasCast = TRUE;
	...
	break;
      case (2) :  // veil
	sn = skill_lookup("veil of darkness");
	if (!is_affected(ch, sn)) {
	  if (get_skill(victim, sn) > 99)
	    spell_veil(sn, victim->level, victim, (void*)ch,TARGET_CHAR);
	  else {
	    act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	  }
	  hasCast = TRUE;
	...
	if (get_skill(victim, sn) > 99)
	  spell_energy_drain(sn, victim->level, victim, (void*)ch,TARGET_CHAR);
	else {
	  act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	}
	hasCast = TRUE;
	...
	if (get_skill(victim, sn) > 99)
	  spell_harm(sn, victim->level, victim, (void*)ch,TARGET_CHAR);
	else {
	  act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	}
	hasCast = TRUE;
	...
	if (get_skill(victim, sn) > 99)
	  spell_magic_missile(sn, victim->level, victim, (void*)ch,TARGET_CHAR);
	else {
	  act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	}
	hasCast = TRUE;
    ...
    act("Your faith reaches out and finds solace as you are assaulted.", victim,NULL,ch,TO_CHAR);
    act("$n is briefly surrounded by a holy aura.", victim,NULL,ch,TO_VICT);
    act("$n is briefly surrounded by a holy aura.", victim,NULL,ch,TO_NOTVICT);
    switch (number)
      {
      case (0) :  // armor/protection
	sn = skill_lookup("armor");
	if (!is_affected(victim, sn)) {
	  if (get_skill(victim, sn) > 99)
	    spell_armor(sn, victim->level,victim, (void*)victim,TARGET_CHAR);
	  else {
	    act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	  }
	  hasCast = TRUE;
	...
	break;
      case (1) : // Virtuous Light
        sn = skill_lookup("virtuous light");
	if (!is_affected(victim, sn)) {
	  if (get_skill(victim, sn) > 99)
	    spell_virtuous_light(sn, victim->level, victim, (void*)victim,TARGET_CHAR);
	  else {
	    act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	  }
	  hasCast = TRUE;
	...
	if (!is_affected(victim, sn)) {
	  if (get_skill(victim, sn) > 99)
	    spell_sanctuary(sn, victim->level, victim, (void*)victim,TARGET_CHAR);
	  else {
	    act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	  }
	  hasCast = TRUE;
	...
	if (get_skill(victim, sn) > 99)
	  spell_cure_poison(sn, victim->level, victim, (void*)victim,TARGET_CHAR);
	else {
	  act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	}
	hasCast = TRUE;
	...
	if (!is_affected(victim, sn))
	  break;
	sn = skill_lookup("cure blind");
	if (get_skill(victim, sn) > 99)
	  spell_cure_blindness(sn, victim->level, victim, (void*)victim,TARGET_CHAR);
	else {
	  act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	}
	hasCast = TRUE;
	...
	if (get_skill(victim, sn) > 99)
	  spell_cure_disease(sn, victim->level, victim, (void*)victim,TARGET_CHAR);
	else {
	  act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
	}
	hasCast = TRUE;
    ...
    if (hasCast == FALSE) {
      ...
      if (get_skill(victim, sn) > 99)
	spell_cure_serious(sn, victim->level, victim, (void*)victim,TARGET_CHAR);
      else {
	act("But your lack of mastery prevents the spell from firing properly.", victim,NULL,ch,TO_CHAR);
      }
    }
  }
  check_improve(victim, gsn_magic_strike, TRUE, 1);
  }
...
void lagprot_msg(CHAR_DATA* ch, CHAR_DATA* victim, int sn, int shield, bool fYell)
{
  ...
  switch (shield)
    ...
    case LAG_SHIELD_PROT:
      ...
      /* WALL OF BONE */
    case LAG_SHIELD_BONE:
      sprintf(to_char, "Your %s is stopped as %s a mass of spinning bone.", noun, verb_tch);
      sprintf(to_vict, "$n's %s %s wall of spinning bone.",noun, verb_tvic);
      sprintf(to_room, "$n's %s %s $N's animated wall.", noun, verb_trm);
      break;
    ...
    case LAG_SHIELD_MALFORM:
      sprintf(to_char, "Your %s is ineffective as %s a flickering magical barrier.", noun,verb_tch);
      sprintf(to_vict, "$n's %s %s weapon's magical barrier.", noun, verb_tvic);
      sprintf(to_room, "$n's %s %s a flickering magical barrier around $N.", noun, verb_trm);
      break;
    ...
    case LAG_SHIELD_BALANCE:
      /* IRON WILL */
    case LAG_SHIELD_IWILL:
      sprintf(to_char, lagattack_table[verb_pos].ba_char);
      sprintf(to_vict, lagattack_table[verb_pos].ba_vict);
      sprintf(to_room, lagattack_table[verb_pos].ba_room);
      break;
    }//END SWITCH
...
int get_lagprot(CHAR_DATA* ch)
{
  ...
  if ( IS_AFFECTED2(ch, AFF_PROTECT_SHIELD) )
  ...
  else if(is_affected(ch, gsn_bone_wall))
    shield_type = LAG_SHIELD_BONE;
    ...
    shield_type = LAG_SHIELD_BALANCE;
	   ...
	   && ch->mana > 0)
    shield_type = LAG_SHIELD_BALANCE;
  else if ( (blade = get_eq_char( ch, WEAR_WIELD)) != NULL &&
	    affect_find(blade->affected, gen_malform) != NULL)
    if (affect_find(blade->affected, gen_final_verdict) != NULL)
      shield_type = LAG_SHIELD_MALFORM;
  return shield_type;
}
...
bool do_lagprot(CHAR_DATA* ch, CHAR_DATA* victim, int chance, int* do_dam, int* lag, int sn, bool fSuccess, bool fYell)
{
  ...
  if ( (shield = get_lagprot(victim)) != LAG_SHIELD_NONE)
      ...
      else if (shield == LAG_SHIELD_ARMOR)
	{
      ...
      else if (shield == LAG_SHIELD_MALFORM)
	{
      ...
      else if (shield == LAG_SHIELD_PROT)
	{
      ...
      else if (shield == LAG_SHIELD_BONE)
	{
	  fProt = TRUE;
	  vic_lag = 0;
	  dam = dam / 4;
	...
	lagprot_msg(ch, victim, sn, shield, fYell);
      *lag = vic_lag;
      *do_dam = dam;
      if (vic_lag > 0 && fSuccess == TRUE)
	check_magic_strike(ch, victim);
      return fProt;
    }//END LAGPROTECTION
  if (vic_lag > 0 && fSuccess == TRUE)
    check_magic_strike(ch, victim);
...
void check_improve( CHAR_DATA *ch, int sn, bool success, int bonus_gain ){
  ...
  if (rating == 0 || *learned < 2 || *learned >= limit)
    return;
  else if (ch->level <= 14 && *learned >= 80)
    return;
  else
...
void do_dirting ( CHAR_DATA *ch, int wait, char *argument) {
	...
	act("$n kicks a handful of dirt through your non-corporeal form.", ch, NULL, victim, TO_VICT);
	chance = 0;
      }
    if (victim->class == gcn_lich) {
      act("You kick a few stones through $N's hollow eyes, to no effect.", ch, NULL, victim, TO_CHAR);
      act("$n kicks a bit of dirt through $N's hollow eyes, to no effect.", ch, NULL, victim, TO_NOTVICT);
      act("$n kicks a handful of dirt through your eye sockets, tickling the back of your skull.", ch, NULL, victim, TO_VICT);
      chance = 0;
    }
    if (number_percent() < chance)
    {
	...
	if (fDirtProt)
	  {
	    send_to_char("Your lizard ancestry serves you well and you easily recover.\n\r", victim);
	    return;
	  }
...
//bigger guys are easier to trip
 	...
 	if (ch->class == class_lookup("monk"))
	{
	    ...
	    if (!(is_affected(victim, gsn_iron_will) && victim->mana > 0) || number_percent() < (25 + victim->level - ch->level)){
	      if (fTrample && ch->size > victim->size) {
		send_to_char("You trample your foe underfoot.\n\r", ch);
		  WAIT_STATE2(victim, ch->size * PULSE_VIOLENCE);
	      }
		else
	  ...
	  if (number_percent() < chance2)
              ...
              if (ch->class == class_lookup("monk") && number_percent() < chance) 
		{
			...
			damage(ch,victim,2*dam/3, gsn_double_kick,DAM_BASH,TRUE);
		      if (victim->position > POS_RESTING && !is_safe(ch, victim))
			damage(ch,victim,2*dam/3, gsn_double_kick,DAM_BASH,TRUE);
		      if ( (paf = affect_find(victim->affected, gsn_mind_disrupt)) == NULL || paf->modifier > -30) {
                  ...
                  else if (is_affected(ch,gsn_horse))
		    {
		      ...
		      victim->move = UMAX(0, victim->move - dam/2);
		  ...
		  else if (is_affected(ch,gsn_crane))
		    {
		      send_to_char("You strike with the precision of the crane.\n\r",ch);
...
bool disarm( CHAR_DATA *ch, CHAR_DATA *victim ){
  ...
  if ( IS_STONED(victim)
	...
	&& is_empowered(ch,0)
	&& number_percent() < (get_skill(ch, gsn_dragon) - 40)) {
      act("The power of the Dragon flows through you, and you overcome the curse.",ch,NULL,victim,TO_CHAR);
    }
    else if (ch->class == class_lookup("blademaster")
	     && is_affected(ch,gsn_ironarm)
	     && number_percent() < (get_skill(ch, gsn_ironarm) - 25)) {
      act("Your arms feel like cords of iron, and you overcome the curse.",ch,NULL,victim,TO_CHAR);
    }
    else{
  ...
  if (obj != NULL 
    ...
    fStr = TRUE;
    skill += (get_skill(victim, gsn_ironarm) - 50) / 2;
  }
  ...
  if (ch->class == class_lookup("blademaster")
      && is_affected(ch,gsn_ironarm)) {
    skill -= (3 * (get_skill(ch, gsn_ironarm))) / 10;
  }
...
//smaller people are easier to haymaker
	...
	if (shield == LAG_SHIELD_PROT || shield == LAG_SHIELD_FFIELD)
	  {
		...
		success = 100;
	      ...
	      {
		act("Striking with great force your headbutt penetrates $N's shield.", ch, NULL, victim, TO_CHAR);
	...
	if (IS_SET(victim->act, ACT_UNDEAD) 
	    ...
	    || check_immune(victim, DAM_BASH, FALSE) == IS_IMMUNE)
	  {
	    ...
	    act("You easily shake off the blow.", ch, NULL, victim, TO_VICT);
...
void check_bone_wall( CHAR_DATA* ch, CHAR_DATA* victim, int multi) {
  int dam;
  if (!is_affected(victim, gsn_bone_wall))
    return;
  
  dam = number_range(victim->level / 2, 3* victim->level / 2);
  dam *= multi;
  damage(victim, ch, dam, gsn_bone_wall, DAM_SLASH, TRUE);
}


int check_phylactery (CHAR_DATA *ch)
{
  OBJ_DATA* hold;
  if (IS_IMMORTAL(ch)) {
    return 60;
...
void update_phylactery (CHAR_DATA *ch, int change)
{
  OBJ_DATA* hold;
  int orig_level;
  if (ch->class != gcn_lich)
    return;
  if ( (hold = get_eq_char(ch, WEAR_HOLD)) != NULL && hold->pIndexData->vnum == OBJ_VNUM_PHYLACTERY) {
    orig_level = hold->level;
    if (hold->level == 60 && change > 0)
      return;
    if (hold->level == 0 && change < 0)
      return;
    hold->level += change;
    if (hold->level >= 60) {
      send_to_char("You feel waves of evil radiate from you as your phylactery reaches its full power.\n\r", ch);
      hold->level = 60;
    }
    if (change > 0) {
      if (orig_level < 20 && hold->level > 20) {
	send_to_char("Your phylactery darkens slightly with its gained power.\n\r", ch);
      }
      else if (orig_level < 40 && hold->level > 40) {
	send_to_char("Your phylactery twitches with dark power.\n\r", ch);
      }
      else if (orig_level < 50 && hold->level > 50) {
	send_to_char("Your phylactery chills as its power grows greater.\n\r", ch);
      }
    }
    else {
      if (orig_level >= 50 && hold->level < 50)
	send_to_char("Your phylactery warms as its power lessens.\n\r", ch);
      else if (orig_level >= 40 && hold->level < 40)
	send_to_char("Your phylactery stills as its power wanes.\n\r", ch);
      else if (orig_level >= 20 && hold->level < 20)
	send_to_char("You feel that your phylactery is severly weakened.\n\r", ch);
    }
    if (hold->level <= 0) {
      hold->level = 0;
    }
