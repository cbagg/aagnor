...
void do_chii_bolt(CHAR_DATA *ch, char *argument)
{
	...
	case (3) :
	  ...
	  if (is_affected(victim, skill_lookup("reflective-field"))){
	      ...
	      affect_strip(victim, skill_lookup("reflective-field"));
	  }
	  ...
	  if (is_affected(victim, skill_lookup("steel wall"))) {
	      act("Your chii bolt shatters the spinning steel around $N.",ch,NULL,victim,TO_CHAR);
	      act("$n's chii bolt shatters the spinning steel around $N.",ch,NULL,victim,TO_NOTVICT);
	      act("$n's chii bolt shatters the spinning steel around you.",ch,NULL,victim,TO_VICT);
	      affect_strip(victim, skill_lookup("steel wall"));
	  }
	  if (is_affected(victim, skill_lookup("fire shield"))) {
	    act("Your chii bolt extinguishes the burning fire around $N.",ch,NULL,victim,TO_CHAR);
	    act("$n's chii bolt extinguishes the burning fire around $N.",ch,NULL,victim,TO_NOTVICT);
	    act("$n's chii bolt extinguishes the burning fire around you.",ch,NULL,victim,TO_VICT);
	    affect_strip(victim,skill_lookup("fire shield"));
	  }
	  if (is_affected(victim, skill_lookup("ice shield"))) {
	    act("Your chii bolt splinters the icy sphere around $N.",ch,NULL,victim,TO_CHAR);
	    act("$n's chii bolt splinters the icy sphere around $N.",ch,NULL,victim,TO_NOTVICT);
	    act("$n's chii bolt splinters the icy sphere around you.",ch,NULL,victim,TO_VICT);
	    affect_strip(victim,skill_lookup("ice shield"));
	  }
	  WAIT_STATE2(victim, 3 * PULSE_VIOLENCE);
...
//EZ
 ...
 if (IS_NPC(victim))
   {
     act("Why would you waste your time on a mere mob?", ch, NULL, victim, TO_CHAR);
     return;
   }
