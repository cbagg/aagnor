...
void do_quest_lich(CHAR_DATA *ch, char *argument )
{
    ROOM_INDEX_DATA* room;
    int sn;
    char arg[MIL];
    ...
    send_to_char("Ok.\n\r",ch);
    send_to_char("The Dark Ritual fills your veins, the ancient magic consuming your soul.\n\r",victim);
    act("$n has been embraced by the Nameless.",victim,NULL,NULL,TO_ROOM);
/* REMORT EFFECTS OF CLASS CHANGE */
  if (victim->level > 50)
    return;
  ...
  /* raise condition if warranted */
  if (victim->pcdata->dall > 10){
    send_to_char("With wonder you see some of the scars disappear from your body.\n\r", victim);
    victim->pcdata->dall -= 10;
  }
  /* reset pkils */
  victim->pcdata->kpc = 0;
 ...
 /* DND SPELLS */
  ...
  sprintf( buf, "the %s", title_table [victim->class] [victim->level] [victim->sex == SEX_FEMALE ? 1 : 0] );
  set_title( victim, buf );
  if ( (room = get_room_index( 3404 )) != NULL){
    char_from_room( victim );
    char_to_room( victim, room);
  }
  set_race_skills(victim, 1);
...
void do_convert( CHAR_DATA *ch, char *argument)
{
  ...
  //now we check things.
  if (fFound && ch->level !=25 && ch->level != 30)
    {
      send_to_char("You may only convert to new calling when at rank 25.\n\rNo more, no less.\n\r", ch);
      return;
    }
