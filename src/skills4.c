...
void do_ventriloquate( CHAR_DATA *ch, char *argument)
{
    ...
    check_improve(ch, skill_lookup("ventriloquate"), TRUE, 1);
}
...
void do_phylactery( CHAR_DATA *ch, char *argument )
{
  ...
  OBJ_DATA *phyl;
  AFFECT_DATA *paf;
  if ((chance = get_skill(ch, gsn_phylactery)) == 0)
    {
      send_to_char("Huh?\n\r", ch);
    }
  if (ch->class != gcn_lich) {
    send_to_char("Only a lich may craft a vile phylactery.\n\r", ch);
    return;
  }
  if (ch->mana != ch->max_mana) {
    send_to_char("You must focus your full power to make a true phylactery.\n\r", ch);
    return;
  }
  if ((phyl = get_obj_carry( ch, "phylactery", ch) != NULL) || ((phyl = get_eq_char(ch, WEAR_HOLD)) != NULL && phyl->pIndexData->vnum == OBJ_VNUM_PHYLACTERY)) {
    send_to_char("You already carry a phylactery.\n\r", ch);
    return;
  }
  ...
  if (number_percent () < chance) {
    ...
    phyl->level = 15;
    obj_to_char(phyl, ch);
    check_improve(ch,gsn_phylactery,TRUE,1);
    ch->mana = 0;
    send_to_char("You drain your mind and pour your soul into a phylactery.\n\r", ch);
  }
  else {
    send_to_char("You fail to focus your power sufficiently to create a functioning phylactery.\n\r", ch);
    ch->mana /= 2;
    check_improve(ch, gsn_phylactery,FALSE,2);
  }
}
...
void do_consume(CHAR_DATA* ch, char *argument)
{
  ...
  if (obj->pIndexData->vnum == OBJ_VNUM_PHYLACTERY) {
    send_to_char("You cannot consume a phylactery into itself.\n\r",ch);
    return;
  }
  else if (obj->pIndexData->vnum == OBJ_VNUM_MAGIC_DISC) {
    send_to_char("You consume the magic disc, and return its power to your phylactery.\n\r", ch);
    update_phylactery(ch, 5);
  }
  else if (obj->pIndexData->vnum == OBJ_VNUM_STARSTONE) {
    send_to_char("In a flash of light you consume the holy starstone.\n\r", ch);
    update_phylactery(ch, 1);
  }
  else {
    int level = obj->level;
    bool rare = CAN_WEAR(obj, ITEM_RARE), unique = CAN_WEAR(obj, ITEM_UNIQUE);
    send_to_char("You consume the raw power in a flash of brilliant light.\n\r", ch);
    if (CAN_WEAR(obj, ITEM_RARE) || CAN_WEAR(obj, ITEM_UNIQUE)) {
      update_phylactery(ch,level/50);
