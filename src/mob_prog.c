...
bool mprog_do_ifchck( char *ifchck, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo, CHAR_DATA *rndm)
{
    ...
    *argpt = '\0';
    ...
    {
	if (atoi(arg) == mud_data.weather_info.sky)
	    return 1;
	else
	    return 0;
    }
    ...
    if ( !str_cmp( buf, "iscabal" ) )
        switch ( arg[1] )
	{
        case 'n': if ( actor && !IS_NPC(actor) && actor->pCabal) return mprog_veval( atoi(val), opr, actor->pCabal->vnum );
                  else return FALSE;
        case 't': if ( vict && !IS_NPC(vict) && vict->pCabal) return mprog_veval( atoi(val), opr, vict->pCabal->vnum );
                  else return FALSE;
        case 'r': if ( rndm && !IS_NPC(rndm) && rndm->pCabal) return mprog_veval( atoi(val), opr, rndm->pCabal->vnum );
                  else return FALSE;
        default:  bug ( "Mob: %d bad argument to 'iscabal'", mob->pIndexData->vnum ); return 0;
        }

// ROYAL IFs - Crypticant
    if ( !str_cmp( buf, "isroyal" ) )
        switch ( arg[1] )
        {
        case 'i': if (IS_SET( mob->act2, PLR_ROYAL )) return TRUE;
                  else return FALSE;
        case 'n': if ( actor )
                      if (IS_SET( actor->act2, PLR_ROYAL )) return TRUE;
                      else return FALSE;
                  else return 0;
        case 't': if ( vict )
                      if (IS_SET( actor->act2, PLR_ROYAL )) return TRUE;
        ...
        default:  bug ( "Mob: %d bad argument to 'isroyal'",mob->pIndexData->vnum ); return 0;
        }        
// END
...
void mprog_driver ( char *com_list, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo)
{
	...
	return;
    /* switched Viri: this should be allowed, especialy for testing etc.
...
void do_old_mpstat( CHAR_DATA *ch, char *argument )
{
    char arg[MIL];
    MPROG_DATA *mprg;
    CHAR_DATA  *victim;
