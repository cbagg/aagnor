...
void spell_extinguish_life (int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
  CHAR_DATA* vch, *vch_next;
  int phyl = check_phylactery(ch), diff = 0;
  int chance = get_skill(ch, sn);
  bool found = FALSE;
  if (phyl < 30) {
    send_to_char("You have nowhere near the power to attempt this.\n\r",ch);
    return;
  }
  if (chance < 75) {
    send_to_char("Without proper skill, this would kill you outright.\n\r",ch);
    return;
  }
  update_phylactery(ch, 29 - phyl);
  for (vch = ch->in_room->people; vch != NULL; vch = vch_next) {
    ...
    if (diff > 25)
      damage(ch,vch,0,sn,DAM_NEGATIVE,TRUE);
    if (!IS_NPC(vch)){
      int dam_pc = dice(2 * (level + (chance - 75) + (phyl - 50)), 4);
      m_yell(ch,vch,FALSE);
      check_dispel(-99, vch, skill_lookup("mana shield"));
      if (!saves_spell(level + 2, vch, DAM_NEGATIVE,skill_table[sn].spell_type))
	damage(ch,vch,dam_pc,sn,DAM_NEGATIVE,TRUE);
      else
	damage(ch,vch,dam_pc/2,sn,DAM_NEGATIVE,TRUE);
      continue;
    }
    else if (diff < -8) {
      act("$N is blasted apart in a wave of negative energy.", ch, NULL, vch, TO_ALL);
      act("$n is DEAD!!", vch, NULL, NULL, TO_ROOM);
      act_new( "You have been KILLED!!", vch, NULL, NULL, TO_CHAR, POS_DEAD);
      raw_kill(ch, vch);
      do_autos(ch);
      update_phylactery(ch, 1);
    }
    else {
      damage(ch, vch, dice (2* (level + (chance - 75)), 5), sn, DAM_NEGATIVE,TRUE); 
    }
  }
  if (found) {
    AFFECT_DATA af;
  ...
  spell_heavens_gate(skill_lookup("portal"), level, ch, NULL, target);
}
...
void spell_bone_armor (int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
  ...
  if (is_affected(victim, sn)) {
    if (victim == ch)
      act("You are already armored in living bone.", ch, NULL, victim, TO_CHAR);
    else
      act("$N is already armored in living bone.", ch, NULL, victim, TO_CHAR);
    return;
  }
  af.where = TO_AFFECTS;
  af.type = sn;
  ...
  if (ch != victim)
    act("Your body chills as you are armored in living bone.", ch, NULL, victim, TO_VICT);
  else
    send_to_char("Your body chills as you are armored in living bone.\n\r", ch);
  act("$N's body takes on a blue hue as $n summons living bone around him.", ch, NULL, victim, TO_ROOM);
}


void spell_dark_heart (int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
  AFFECT_DATA af;
  if (is_affected(ch, sn)) {
    send_to_char("Your heart is already black as pitch.\n\r",ch);
    return;
  }
  af.where = TO_AFFECTS;
  af.type = sn;
  ...
  af.bitvector = 0;
  affect_to_char(ch, &af);
  send_to_char("You purge any traces of goodness from your heart.\n\r", ch);
}


void spell_grasp_crypt (int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
  AFFECT_DATA af;
  CHAR_DATA * victim = (CHAR_DATA*)vo;
  int phyl = check_phylactery(ch), dam = dice(10,phyl/3);
  if (IS_AFFECTED(victim, AFF_FLYING) && !is_affected(victim,gsn_thrash)){
    send_to_char("Rotting hands reach from the ground, but you float above them\n\r",victim);
    act("Rotting hands stretch from the earth at $N, but he floats above them.",ch, NULL, victim, TO_CHAR);
    return;
  }
  if (saves_spell(level, victim, DAM_NEGATIVE, skill_table[sn].spell_type)
      && !is_affected(victim, gsn_ptrium)) {
    dam /= 2;
  }
  else if (is_affected(victim, gsn_enfeeblement)) {
    send_to_char("His legs have already collapsed.\n\r",ch);
  }
  else if (is_affected(victim, gsn_ptrium)) {
    send_to_char("Some power defeats the undead legion.\n\r",ch);
    send_to_char("Your divine strength shakes off the unliving legion.\n\r", victim);
    return;
  }
  else {
    ...
    af.modifier = -1 * (level / 12) - phyl / 30;
    affect_to_char(victim, &af);
    act("You feel the rotting hands drain your legs.",victim, NULL, victim, TO_CHAR);
    act("Rottings hands reach up from the ground and tear at $N's legs.", ch, NULL, victim, TO_ROOM);
    act("You command the legions of unliving to tear at $N's legs.", ch, NULL, victim, TO_CHAR);
  }
  damage(ch,victim,dam,sn, DAM_NEGATIVE, TRUE);
}


void spell_wraith_swarm (int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
  AFFECT_DATA af;
  CHAR_DATA * victim = (CHAR_DATA*)vo;
  int phyl = check_phylactery(ch), dam = dice(5, 10) + (phyl / 20) * level;
  int dur = 4;
  
  if (phyl < 25) {
    send_to_char("Your phylactery is not powerful enough to summon the wraiths.\n\r", ch);
    return;
  }
  if (is_affected(victim, sn)) {
    send_to_char("He is already being swarmed by wraiths.\n\r", ch);
    return;
  }
  act("With a howl of rage, a swarm of wraiths decend on $n!", victim, NULL, victim, TO_ROOM);
...
void spell_cloak_shades (int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
  AFFECT_DATA af;
  if (is_affected( ch, sn)) {
    send_to_char("You are already cloaked in the shades.\n\r", ch);
    return;
  }
  if (check_phylactery(ch) < 20) {
    send_to_char("Your phylactery is too weak to summon the shades.\n\r", ch);
    return;
  }
  send_to_char("Shades flow from cracks in the ground, forming a dark cloak around you.\n\r.", ch);
  act("$n is enveloped in a horde of shades.", ch, NULL, NULL, TO_ROOM);
  af.where = TO_AFFECTS;
  af.type = sn;
  af.level = level;
  af.location = APPLY_SAVING_MENTAL;
  af.modifier = - (check_phylactery(ch) / 10);
  af.duration = check_phylactery(ch);
  af.bitvector = 0;
  affect_to_char(ch, &af);
}


void spell_eyes_grave ( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
  AFFECT_DATA af;
  if (is_affected( ch, sn) ) {
    send_to_char("You already see all, living and dead.\n\r", ch);
    return;
  }
  if (check_phylactery(ch) < 10) {
    send_to_char("Your power has drained too far to see the grave.\n\r", ch);
    return;
  }
  update_phylactery(ch, -2);
  ...
  af.where = TO_AFFECTS;
  affect_to_char (ch, &af );
  act("Small points of red light appear at the center of $n's skull.", ch, NULL, NULL, TO_ROOM);
  act("Your hollow sockets tingle, and the world is thrown into contrast.", ch, NULL, NULL, TO_CHAR);
}

void spell_magic_disc ( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
  OBJ_DATA * disc;
  int phyl = check_phylactery(ch);
  if (phyl < 5) {
    send_to_char("You do not have the power to craft a floating disc",ch);
    return;
  }
  disc = create_object( get_obj_index( OBJ_VNUM_MAGIC_DISC ), 0);
  disc->level = level;
  disc->timer = phyl * 3;
  update_phylactery(ch, -5);
  obj_to_char(disc,ch);
  act("You summon a magical disc.", ch, NULL, NULL, TO_CHAR);
  act("$n forms a disc out of the essense of magic.", ch, NULL, NULL, TO_ROOM);
}

void spell_bone_spear( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  CHAR_DATA *victim = (CHAR_DATA *)vo;
  AFFECT_DATA af;
  int phyl = check_phylactery(ch), dam = (7 * level / 2) + (phyl / 5) * (dice (3, (phyl / 10)) + 3);
  int skill = get_skill(ch, sn);
  if (ch->class != gcn_lich) {
    send_to_char("Only a lich may channel forth spears of bone.\n\r", ch);
    return;
  }
  if (phyl < 30) {
    send_to_char("Your phylactery sputters, and cannot muster the energy to throw a spear of bone.\n\r", ch);
    return;
  }
  if (!is_affected(ch, gsn_bone_wall)) {
    send_to_char("Without a wall of bone, you cannot draw a spear from it.\n\r", ch);
    return;
  }
  if (is_affected(ch, gsn_bone_spear)) {
    send_to_char("You cannot deplete your wall further.\n\r", ch);
    return;
  }
  if ( saves_spell((level/2 + phyl/2), victim, DAM_PIERCE, skill_table[sn].spell_type))
    dam /= 2;
  act("You send forth a shaft of bone from your wall, piercing $N.", ch, NULL, victim, TO_CHAR);
  act("A spear of bone detaches itself from $n's wall, piercing you through.", ch, NULL, victim, TO_VICT);
  act("A spear of bone detaches itself from $n's wall, piercing $N through.", ch, NULL, victim, TO_ROOM);
  af.where = TO_AFFECTS;
  af.type = sn;
  af.level = level;
  af.duration = 3 - phyl/20;
  af.location = 0;
  af.modifier = 0;
  af.bitvector = 0;
  affect_to_char(ch, &af);
  update_phylactery(ch, -3);
...
void spell_hypnosis( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    if (get_curr_stat(ch, STAT_INT ) + (phyl / 20) < get_curr_stat(victim, STAT_INT))
    {
...
//Misc factors
  ...
  if ( ((ch->alignment > EVIL_THRESH)&& deity_table[ch->pcdata->way].shrine != 2530) || IS_SET(ch->act, PLR_OUTCAST) 
...
void spell_mimic( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    if (IS_AFFECTED2(ch,AFF_FIRE_SHIELD)) {
      send_to_char("The magical flames about you destroy your attempt to create the reflective shield.\n\r", ch);
      return;
    }
    af.type   = sn;
    af.level  = level;
...
void spell_bone_wall (int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  CHAR_DATA *vch, *vch_next;
  AFFECT_DATA af;
  int dam, phyl = check_phylactery(ch);
  if ( is_affected (ch, gsn_bone_wall))
    {
      dam = number_range(level + phyl, 3 * (level + phyl) / 2 );
      for (vch = ch->in_room->people; vch != NULL; vch = vch_next)
	{
	  vch_next = vch->next_in_room;
	  if (vch != ch && !is_area_safe(ch,vch))
	    {
	      m_yell(ch,vch,FALSE);
	      if ( saves_spell( phyl, vch, DAM_PIERCE, SPELL_AFFLICTIVE) )
		dam = 3 * dam / 5;
  ...
  af.type = sn;
  af.level = level;
  af.duration = number_fuzzy(level/10) + phyl/10;
  af.location = 0;
  af.modifier = 0;
  af.bitvector = 0;
  affect_to_char( ch, &af);
  act( "You surround yourself with a spinning wall of bone.", ch, NULL, NULL, TO_CHAR);
  act( "A wall of spinning bone surrounds $n", ch, NULL, NULL, TO_ROOM);
}


void spell_soul_tear (int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  OBJ_DATA * hold;
  AFFECT_DATA af;
  AFFECT_DATA* paf;
  int dam = 10 + 2 * level, gain = victim->level/25;
  if (IS_NPC(victim)) {
    send_to_char("You feel that this soul is not worth shredding.\n\r", ch);
    return;
  }
  if ((hold = get_eq_char(ch, WEAR_HOLD)) != NULL && hold->pIndexData->vnum == OBJ_VNUM_PHYLACTERY) {
    if (is_affected(victim, gsn_soul_tear)) {
      send_to_char("This soul is too tattered to feed your lust.\n\r", ch);
      return;
    }
    if (is_affected(victim, gsn_ptrium)) {
      send_to_char("Something goes horribly wrong, and you feel your power richochet upon yourself.\n\r", ch);
      send_to_char("As the final words are spoken, the power of the One fills your flesh, and sustains you.\n\r", victim);
      update_phylactery(ch, -10);
      damage(ch, ch, dam*2, sn, DAM_INTERNAL, TRUE);
      return;
    }
    else if (is_affected(victim, gsn_buddha)) {
      send_to_char("There seem to be no weaknesses in this soul.\n\r", ch);
      send_to_char("You feel a tickling at the edge of your tranquil state, and push it away.\n\r", victim);
      return;
    }
    send_to_char("You feel your soul torn apart and ripped towards the upraised phylactery.\n\r", victim);
    if (victim->alignment >= GOOD_THRESH) {
      if (IS_AVATAR(victim)) {
	send_to_char("You feel a giddy rush of power as you steal a portion of an Avatar's soul.\n\r",ch);
	gain*=4;
      }
      else {
	send_to_char("You feel a rush of power as you steal a portion of a goodly being's soul.\n\r",ch);
	gain*=2;
      }
    }
    else if (victim->alignment <= EVIL_THRESH) {
      send_to_char("You feel no power from a soul already consumed in darkness.\n\r",ch);
      gain = 0;
    }
    else {
      send_to_char("You feel power from shattering a soul.\n\r", ch);
    }
    damage(ch, victim, dam, sn, DAM_INTERNAL, TRUE);
    update_phylactery(ch, gain);
    af.where = TO_AFFECTS;
    af.type = gsn_soul_tear;
    af.level = level;
    af.duration = 24;
    af.bitvector = 0;
    af.modifier = 10;
  ...
  else if (victim->class == gcn_lich && ch->class != gcn_lich) { //spell turned
    send_to_char("The tearing rebounds upon your spell shield with terrific force.\n\r", ch);
    send_to_char("You feel your negative energies returning upon you with tremendous force.\n\r", victim);
    damage(ch, victim, dam*2, sn, DAM_INTERNAL, TRUE);
    return;
  }
  else {
    send_to_char("Without a phylactery to recieve the shattered soul, there is no purpose to the pain.\n\r", ch);
  }
}
