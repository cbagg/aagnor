...
*/
    ...
    if ( CAN_WEAR( obj, ITEM_WEAR_SHROUD ) && (ch->class == class_lookup("necromancer") || IS_IMMORTAL(ch) || ch->class == gcn_lich))
    {
...
void do_sell( CHAR_DATA *ch, char *argument ){
  ...
  CHAR_DATA *keeper;
  OBJ_DATA *obj;
  int cost,haggle_cost, roll = number_percent();
  one_argument( argument, arg );
  ...
  if ( arg[0] == '\0' ){
	...
	&& roll < get_skill(ch,gsn_haggle)){
      ...
      haggle_cost = UMAX(0, roll * (obj->cost - cost) / 100);
      cost += haggle_cost;
