...
bool shoot( CHAR_DATA *ch, CHAR_DATA* victim, int skill, bool fArchery, bool fQuiet ){
  ...
  if (get_skill(victim, gsn_shield_expert) > 1
      && get_eq_char(ch, WEAR_SHIELD) != NULL) {
    send_to_char("You raise your shield, and hear the missiles clatter off uselessly.\n\r",victim);
    dam = 0;
  }
  dam = UMAX(0, dam );
...
/* HIT */
	...
	&& ch->pcdata->condition[COND_HUNGER] < 40 
      && (ch->race == race_lookup("undead") || ch->race == race_lookup("lich"))){
...
//MANTIS CHECK
  ...
  if (fSuccess){
    ...
    //STOMACH
    case 1:
      victim->move = UMAX(0, victim->move - 2 * dam / 3);    
      break;
	...
	affect_to_char(victim, &af);
      ...
      //HEAD
    case 3:
      victim->mana = UMAX(0, victim->mana - 2 * dam / 3);
      break;
