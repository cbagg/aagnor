...
void spell_dispel_magic( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  /* check saves */
  if (is_affected(victim, skill_lookup("mana shield")) && ch->class == gcn_lich && check_phylactery(ch) > 40) {
    send_to_char("You focus the dark power of your phylactery on the mana shield.\n\r", ch);
    check_dispel(-99, victim, skill_lookup("mana shield"));
    return;
  }
  if (ch == victim 
...
void spell_corpse_explosion( int sn,int level,CHAR_DATA *ch, void *vo,int target )
{
    ...
    int dur=0;
    if (obj->item_type != ITEM_CORPSE_PC || obj->condition < 2)
    {
        send_to_char("You can't make that explode.\n\r",ch);
        return;
    }
    if (obj->level<15)
    {
    ...
    act( "A corpse slowly rises into the air and `1EXPLODES`` showering the room with entrails!", ch, NULL, NULL, TO_ROOM );
    act( "You cause the corpse to rise into the air and `1EXPLODE!``.", ch, NULL, NULL, TO_CHAR );
    free_string(obj->description);
    obj->description = str_dup("The hollow bloody remains of a corpse is here.");
    obj->condition=1;
    for ( ich = ch->in_room->people; ich != NULL; ich = ich->next_in_room )
    {
    if ( !is_affected( ich, gsn_plague ) )
      {
        if (ch == ich || saves_spell(level,ich,DAM_DISEASE,skill_table[gsn_plague].spell_type) || (IS_NPC(ich) && IS_SET(ich->act,ACT_UNDEAD)))
        {
          act_new("You feel momentarily ill, but it passes.",ich,NULL,NULL,TO_CHAR,POS_DEAD);
          if (ch != ich)
          act("$N seems to be unaffected.",ch,NULL,ich,TO_CHAR);
        }
        else
        {
          dur = level / 10 + (skill > 77? 1: 0) + (skill > 84? 1: 0) + (skill > 88? 1: 0) + (skill > 92? 2: 0) + (skill > 95? 2: 0) + + (skill > 99? 3: 0);	
          af.where     = TO_AFFECTS;
          af.type      = gsn_plague;
          af.level     = level;
          af.duration  = dur;
          ...
          af.modifier  = 0;
          af.bitvector = 0;
          paf = affect_to_char(ich,&af);
          if (!IS_NPC(ch)) string_to_affect(paf, ch->name);
          act_new("You scream in agony as plague sores erupt from your skin.",ich,NULL,NULL,TO_CHAR,POS_DEAD);
          act("$n screams in agony as plague sores erupt from $s skin.",ich,NULL,NULL,TO_ROOM);
        }
      }    
    }
}
...
/* cap to ignore spell level boost */
    ...
    if (get_summoned(ch, MOB_VNUM_SKELETON) > 0)
    {
        send_to_char("You already control a skeleton.\n\r",ch);
        return;
    }
    if ( is_affected( ch, sn ) )
    {
        send_to_char("You do not feel strong enough to summon another skeleton.\n\r",ch);
        return;
    }
    af.where     = TO_AFFECTS2;
    af.type      = sn;
    ...
    af.bitvector = 0;
    affect_to_char( ch, &af );
    act ("The ground opens and a skeleton ascends from the earth!", ch, NULL, NULL, TO_CHAR);
    act ("$n summons a skeleton from within the earth!", ch, NULL, NULL, TO_ROOM);
...
void spell_wrath(int sn, int level, CHAR_DATA *ch, void *vo,int target){
  ...
  if ( is_affected(ch,gsn_heroism) )
    dam = 4 * dam / 3;
  if ( victim->class == gcn_lich && check_phylactery(victim) == 60
       && is_affected(victim, skill_lookup("dark heart")))
    dam /= 2;
...
void spell_flashfire( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	if (!IS_NPC(vch) && number_percent() < get_skill(vch,gsn_blind_fighting)){
	  if (is_affected(vch, gsn_battletrance) || is_affected(vch, gsn_insight))
	    af.modifier      = 0;
	  else
	    af.modifier      = -3;
	}
...
void spell_heavens_gate( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
	...
	else if (ch->class == gcn_lich && check_phylactery(ch) < 50
		 && !is_affected(ch, skill_lookup("extinguish life"))) {
	  send_to_char("Your phylactery is too weak to propel you through the portal.\n\r", ch);
	  return;
	}
...
void spell_morph( int sn, int level, CHAR_DATA *ch, void *vo,int target) 
{
    ...
    AFFECT_DATA af;
    target_name = one_argument(target_name, arg);
    if (is_affected(ch,gsn_planar_seal)) {
      send_to_char("You have been sealed to your current form.\n\r",ch);
      return;
    }
