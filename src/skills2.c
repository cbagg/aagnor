...
void do_herb( CHAR_DATA *ch, char *argument)
{
    ...
    gherb = get_skill(ch, gsn_group_herb );
    one_argument (argument,arg);
...
void do_beast_call( CHAR_DATA *ch, char *argument)
{
    ...
    bool char_evil = FALSE, char_neutral = FALSE, char_good = FALSE;
    bool have_mammoth = FALSE, have_leopard = FALSE, have_displacer = FALSE, have_wyvern = FALSE;
    bool have_manticore = FALSE, have_nightmare = FALSE, have_umberhulk = FALSE;
    bool have_basilisk = FALSE, have_hippogryph = FALSE, have_gorgon = FALSE;
	...
	beast = MOB_VNUM_DISPLACER;
      else if (!str_prefix(argument, "manticore"))
        beast = MOB_VNUM_MANTICORE;
      else if (!str_prefix(argument, "nightmare"))
        beast = MOB_VNUM_NIGHTMARE;
      else if (!str_prefix(argument, "umber"))
        beast = MOB_VNUM_UMBERHULK;
      else if (!str_prefix(argument, "basilisk"))
        beast = MOB_VNUM_BASILISK;
      else if (!str_prefix(argument, "hippogryph"))
        beast = MOB_VNUM_HIPPOGRYPH;
      else if (!str_prefix(argument, "gorgon"))
        beast = MOB_VNUM_GORGON;
      else{
        ...
        if (IS_AFFECTED (vch, AFF_CHARM) && vch->master == ch)
	{
	  ...
	  if (IS_NPC(vch) && vch->pIndexData->vnum == MOB_VNUM_RAVEN){
	    ...
	    have_displacer = TRUE;
          ...
          else if (IS_NPC(vch) && vch->pIndexData->vnum == MOB_VNUM_MANTICORE){
            i++;
            have_manticore = TRUE;
          }
          else if (IS_NPC(vch) && vch->pIndexData->vnum == MOB_VNUM_NIGHTMARE){
            i++;
            have_nightmare = TRUE;
          }
          else if (IS_NPC(vch) && vch->pIndexData->vnum == MOB_VNUM_UMBERHULK){
            i++;
            have_umberhulk = TRUE;
          }
          else if (IS_NPC(vch) && vch->pIndexData->vnum == MOB_VNUM_BASILISK){
            i++;
            have_basilisk = TRUE;
          }
          else if (IS_NPC(vch) && vch->pIndexData->vnum == MOB_VNUM_HIPPOGRYPH){
            i++;
            have_hippogryph = TRUE;
          }
          else if (IS_NPC(vch) && vch->pIndexData->vnum == MOB_VNUM_GORGON){
            i++;
            have_gorgon = TRUE;
          }
	}
	...
	check_improve(ch,gsn_beast_call,TRUE,1);
        ...
        if (beastroll > 90 && ch->level >= 40 && char_neutral == TRUE && !have_mammoth && (beast == 0 || beast == MOB_VNUM_MAMMOTH))
        {
	...
	else if (beastroll > 90 && ch->level >= 40 && char_evil == TRUE && !have_umberhulk && (beast == 0 || beast == MOB_VNUM_UMBERHULK))
        {
            act("An umber hulk thunders to your side.",ch,NULL,NULL,TO_CHAR);
            act("An umber hulk thunders to $n's side.",ch,NULL,NULL,TO_ROOM);
            mob = create_mobile( get_mob_index( MOB_VNUM_UMBERHULK));
            mob->max_hit        = ch->level * 13;
            mob->hitroll        = ch->level/5 + 7;
            mob->damroll        = 2*ch->level/5 + 7;
            mob->damage[DICE_NUMBER]    = 4;
	...
	else if (beastroll > 90 && ch->level >= 40 && char_good == TRUE && !have_gorgon && (beast == 0 || beast == MOB_VNUM_GORGON))
        {
            act("A gorgon thunders to your side.",ch,NULL,NULL,TO_CHAR);
            act("A gorgon thunders to $n's side.",ch,NULL,NULL,TO_ROOM);
            mob = create_mobile( get_mob_index( MOB_VNUM_GORGON));
            mob->max_hit        = ch->level * 13;
            mob->hitroll        = ch->level/5 + 7;
            mob->damroll        = 2*ch->level/5 + 7;
            mob->damage[DICE_NUMBER]    = 4;
        ...
        else if (beastroll > 70 && ch->level > 35 && char_neutral == TRUE && !have_leopard && (beast == 0 || beast == MOB_VNUM_LEOPARD))
        {
        ...
        else if (beastroll > 70 && ch->level > 35 && char_good == TRUE && !have_hippogryph && (beast == 0 || beast == MOB_VNUM_HIPPOGRYPH))
        {
            act("A hippogryph comes to your rescue.",ch,NULL,NULL,TO_CHAR);
            act("A hippogryph arrives to heed $n's call.",ch,NULL,NULL,TO_ROOM);
            mob = create_mobile( get_mob_index( MOB_VNUM_HIPPOGRYPH));
            mob->max_hit        = ch->level * 12;
            mob->hitroll        = ch->level/5 + 12;
            mob->damroll        = ch->level/5 + 5;
            mob->damage[DICE_NUMBER]    = 4;
        ...
        else if (beastroll > 70 && ch->level > 35 && char_evil == TRUE && !have_nightmare && (beast == 0 || beast == MOB_VNUM_NIGHTMARE))
        {
            act("A nightmare comes to your rescue.",ch,NULL,NULL,TO_CHAR);
            act("A nightmare arrives to heed $n's call.",ch,NULL,NULL,TO_ROOM);
            mob = create_mobile( get_mob_index( MOB_VNUM_NIGHTMARE));
            mob->max_hit        = ch->level * 12;
            mob->hitroll        = ch->level/5 + 12;
            mob->damroll        = ch->level/5 + 5;
            mob->damage[DICE_NUMBER]    = 4;
        ...
        else if (beastroll > 50 && ch->level >= 30 && char_neutral == TRUE && !have_wyvern && (beast == 0 || beast == MOB_VNUM_WYVERN))
        {
        ...
        else if (beastroll > 50 && ch->level >= 30 && char_good == TRUE && !have_basilisk && (beast == 0 || beast == MOB_VNUM_BASILISK))
        {
            act("A greater basilisk comes to your side.",ch,NULL,NULL,TO_CHAR);
            act("A greater basilisk comes to $n's side.",ch,NULL,NULL,TO_ROOM);
            mob = create_mobile( get_mob_index( MOB_VNUM_BASILISK));
            mob->max_hit        = ch->level * 11;
            mob->hitroll        = ch->level/5 + 10;
            mob->damroll        = ch->level/5;
            mob->damage[DICE_NUMBER]    = 4;
        ...
        else if (beastroll > 50 && ch->level >= 30 && char_evil == TRUE && !have_manticore && (beast == 0 || beast == MOB_VNUM_MANTICORE))
        {
            act("A manticore comes to your side.",ch,NULL,NULL,TO_CHAR);
            act("A manticore comes to $n's side.",ch,NULL,NULL,TO_ROOM);
            mob = create_mobile( get_mob_index( MOB_VNUM_MANTICORE));
            mob->max_hit        = ch->level * 11;
            mob->hitroll        = ch->level/5 + 10;
            mob->damroll        = ch->level/5;
            mob->damage[DICE_NUMBER]    = 4;
...
/* Allow thieves, ninjas, bards, to be able to throw any level items
   - Arzaeth -
...
void do_throw( CHAR_DATA *ch, char *argument )
{
    ...
    if ( shield == LAG_SHIELD_PROT || shield == LAG_SHIELD_FFIELD || shield == LAG_SHIELD_BONE)
    {
...
//Blinding hit!
      else if (chance <= 80)
	{
	  if (!is_affected(victim, gsn_blindness))
		...
		if (is_affected(victim, gsn_battletrance) || is_affected(victim, gsn_insight))
		  af.modifier      = 0;
		else
		  af.modifier      = -5;
	      }
